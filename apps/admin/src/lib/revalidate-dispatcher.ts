 ...
 */
export async function dispatchRevalidation({ tags = [], paths = [] }: RevalidateOptions): Promise<void> {
  const publicSiteUrl = process.env.NEXT_PUBLIC_SITE_URL;
  const secret = process.env.REVALIDATION_SECRET_TOKEN;

  if (!publicSiteUrl || !secret) {
    console.warn('[revalidate] Missing NEXT_PUBLIC_SITE_URL or REVALIDATION_SECRET_TOKEN — skipping revalidation.');
    return;
  }
